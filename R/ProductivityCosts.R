...
calculateProdCost <- function(TbCases,
  ...
  DUR_TB_prTX <- 3*30 # average duration of TB prior to treatment (3 months)
  ...
  if (discount == 0){
    ...
    annual_expend <- readRDS(system.file(paste0("NonHealthExpenditure", Ages, ".rds"), package = "TubercuCost")) #converted to 2021 in csv
    ...
    annual_expend <- readRDS(system.file(paste0("DiscountedNonHealthExpenditure", Ages, ".rds"), package = "TubercuCost")) #converted to 2021 in csv
